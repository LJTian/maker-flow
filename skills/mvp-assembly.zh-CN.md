...
写到**当前产品仓根目录**（由 `maker-flow new` / `init` 创建）。见 `docs/consumer-project.md` 与 `AGENTS.consumer.example.md`。

`<项目名>` 由 PRO 摘要推导（kebab-case，如 `todo-api`）。

**禁止**把组装 MVP 代码写入 maker-flow 工厂仓。若工作树是工厂（无产品 `AGENTS.md`），先创建产品仓（`maker-flow new <名字>`），或一次性冒烟用 `/tmp/maker-flow-smoke/<项目名>/`。
