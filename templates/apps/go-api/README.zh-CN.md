...
- CORS（环境变量 `CORS_ORIGINS`，默认 `*`）、结构化日志、panic recover
...
- 与 `web-vite` 联用见布局 [`web-api`](../../layouts/web-api/)；拆分上线：`skills/publish-split-web-api.md`
...
3. 遵循 `skills/mvp-assembly.md`。可选 patterns：见 `templates/patterns/index.md`。需要数据库/表时用 [`persistence-sqlx`](../../patterns/persistence-sqlx/)。
